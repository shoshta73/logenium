...
def generate_protocol(protocol_path: Path, base_name: str) -> bool:
    header_output = Directories.xheader_include / "xheader" / "internal" / f"{base_name}-client-protocol.h"
    source_output = Directories.xheader_source / "internal" / f"{base_name}-protocol.c"
...
def main(ctx: typer.Context) -> None:
    ...
    protocols = [
        (Path("/usr/share/wayland-protocols/stable/xdg-shell/xdg-shell.xml"), "xdg-shell"),
        (
    ...
    for protocol_path, base_name in protocols:
        if not protocol_path.exists():
            typer.echo(
                f"{typer.style('[WARNING]', fg='yellow')} Protocol file not found: {protocol_path}",
                err=True,
            )
            typer.echo("Skipping...", err=True)
            continue

        typer.echo(f"\nProcessing {base_name}...")
        if not generate_protocol(protocol_path, base_name):
            raise typer.Exit(1)
